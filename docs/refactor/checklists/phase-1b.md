...
- [ ] **1. Extract config into `bin/lib/dx-config.sh`** — root discovery, the
  canonical config field registry, defaults, origin tracking, retired-variable
  validation, and pure validators.

- [ ] **2. Implement [D2](../decisions/D2-config.md)** with one non-evaluating
...
- [ ] **3. Make config initialization explicit and idempotent.** Resolve and record
...
- [ ] **4. Remove `set -euo pipefail` from every file under `bin/lib/`**, leaving it
...
- [ ] **5. Move Apple Container functions into `dx-container.sh`** and replace
...
- [ ] **6. Prefer `container ... list --quiet` or structured output** when
...
- [ ] **7. Move pure helpers into `dx-host-util.sh`** — name/slug/port derivation,
...
- [ ] **8. Centralize the SSH endpoint and base options in `dx-ssh-common.sh`.**
  Callers may append command-specific options such as `BatchMode` or
  `ExitOnForwardFailure`.

- [ ] **9. Implement [D6](../decisions/D6-command-boundaries.md) across the
...
- [ ] **10. Keep `dx-lib.sh` as a short compatibility facade** while callers migrate
