...
2. **Phase 2 — isolated 26.05 source and build.**
   ...
   - **Status (2026-07-04):** executed in a dedicated worktree — context rename, flake input bump (`nixpkgs`/`nixvim`/`home-manager` → the `26.05`/`release-26.05` branches), targeted lockfile regeneration, `home.stateVersion` bump, the base-image alignment-rule Containerfile bump (`nixos/nix` `2.31.5` → `2.34.7`, re-verified digest), host defaults, and the doc/test release-string sweep all landed in one channel-bump commit. Two pre-existing-in-26.05 packaging breaks were resolved: (1) `neofetch` removed from nixpkgs — replaced its entry in `flake.nix:65` with its maintained successor `fastfetch` (version 2.63.1 verified available at locked 26.05 revs); (2) ncurses 6.6 now provides the ghostty terminfo entry, conflicting with the explicit `ghostty.terminfo` in dxPackages (build-time home-manager buildEnv path conflict; undetectable by `nix flake check` which cannot catch buildEnv path conflicts, discovered in live activation) — resolved by removing the now-redundant `ghostty.terminfo` entry from `flake.nix:66`. `nix flake check` now passes at the locked 26.05 revs. A third pre-existing-in-26.05 break surfaced only in live guest testing (the isolated `dx-test` Stage-B canary — see the Status note on Phase 3 below): (3) nushell 0.112 (shipped by 26.05) removed the `$nu.home-path` constant — the current column is `$nu.home-dir` — so the generated `env.nu` (from `container/aarch64-darwin-apple-container-dx-nixos-26.05/home/shell.nix`) aborted immediately on `source` with `column_not_found`, breaking `$env.PERSIST`/`$env.SSL_CERT_FILE` exposure; caught by live sections 15/16, not by `nix flake check` or the prior static asserts (undetectable by build-time checks since the failure is a nushell runtime API change) — resolved by renaming to `$nu.home-dir` in `home/shell.nix` and updating the section-15 static assertions to match. Live-guest validation *was* performed — but through the isolated `dx-test` Stage-B canary and the destructive fresh-rebuild changeover, not the separately-specified `nixos-2605.env` parallel-profile path in Phase 3 below (that profile was never created). See the Phase 3 Status note.
...
3. **Phase 3 — isolated 26.05 live guest validation.**
   ...
   - **Status (2026-07-05): superseded by the executed strategy.** The
     `nixos-2605.env` parallel-profile path described here was never created.
     Live 26.05 validation was instead performed with the isolated `dx-test`
     profile as the canary (Stage A on the official base at 25.11, then Stage
     B rebuilt from scratch on 26.05 — full profile-aware suite green, release
