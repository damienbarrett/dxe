...
UNEXPECTED_STALE=$(printf '%s\n' "$STALE_MATCHES" | grep -vE \
    'bin/dx-lib.sh|bin/dx-create-volumes|bin/dx-migrate-persist|README.md|tests/test_section9_host_scripts.sh|tests/test_section10_docs.sh|tests/test_section16_persist_storage.sh' || true)
