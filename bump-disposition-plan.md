...
### Gate status — frozen tip `03c4f5c`, 2026-08-31

All gates below were run against candidate tip `03c4f5c` (lock commit
`4ccb57b` plus the waiver) on `main` at `842b287`. The candidate differs from
`main` in exactly two files: `flake.lock` and `docs/release-maintenance.md`.

| Gate | Result |
| --- | --- |
| A — mechanical lock audit | **PASS**, 9 assertions, fail-closed |
| B — `tests/release-check.sh` | **PASS** |
| B — `bash -n`, `/bin/bash` 3.2 syntax | **PASS** |
| B — `tests/run-tier.sh unit/static` | **PASS** |
| B — `tests/run-tier.sh host-contract` | **PASS** |
| B — `tests/run-bash32-tests.sh` | **PASS** |
| B — ShellCheck 0.10.0 (in guest) | **PASS** — zero warning-severity findings repo-wide |
| B — `tests/run-coverage-linux.sh` | **PASS** — `covered=100% scope_share=21.78%` |
| C — `nix flake check` | **PASS** — `all checks passed!` |
| C — four aarch64-linux output builds | **PASS** — lock hash unchanged after build |
| D — reused-volume canary | **PASS** — 1045 passed / 0 failed / 9 skipped |
| D — fresh-volume canary | **PASS** — factory reset, cold rebuild, then 1045 / 0 / 9 |
| Running-generation proof | **PASS** on all three boots |

The running-generation proof was taken three times, on three distinct
generations, because each rebuild minted a new one:

| Boot | Generation | Lock |
| --- | --- | --- |
| Reused volume, pre-bootstrap-changes | `20260830T021223Z-84395` | `ee4d64dc…` |
| Reused volume, frozen tip | `20260830T232037Z-83544` | `ee4d64dc…` |
| Fresh volume, after factory reset | `20260830T232623Z-18149` | `ee4d64dc…` |

The second matters because `ca265ec` changed three `bootstrap/` files after the
first boot; the third is a genuine cold build, corroborated by the SSH keypair
having been regenerated by the reset.
...
`authorized_keys` matches whichever worktree last bootstrapped it. `dx-ssh`
and `dx-put` therefore succeed from `dxe-bump` and fail from the primary
worktree. An earlier revision of this plan recorded that as a key mismatch
blocking Gate D; that was wrong, and the cause was running the command from
the wrong worktree — exactly the failure the resource preflight exists to
catch.
